...
if "last_chat_time" not in st.session_state:
    st.session_state.last_chat_time = time.time()-5

# 5秒以内の連投をブロック
current_time = time.time()
if current_time - st.session_state.last_chat_time < 5:
    st.warning("連投は控えてください（5秒待機が必要です）")
    st.stop()
    ...
    with st.form("answer_form"):
        ...
        if st.form_submit_button("送信"):
            if answer.strip().upper() == CORRECT_ANSWER.strip().upper():
                st.balloons()
                st.success("正解です！Stage 2 クリア！\n次のリンクから Stage 3 へ進もう！")
                st.link_button("Stage 3 はこちら", "https://appmenuchal3kxc2s-mchvzqx3unfb5oydfgnivg.streamlit.app/")
            else:
                st.error("❌ 不正解です。")
